...
Our R2 value indicates that 92% of the variability in our test data can be explained by our model, which is quite high.

```{r}
RMSE(predictions, test$winner_total_points_won)
```

Our RMSE (Root Mean Squared Error) value tells us the average difference between the test values and our predicted values. This indicates that our model's predictions for winner_total_points_won are off by an average of 7 for our test data.
